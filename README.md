...
*How nice would it be to control two or three NeoPixel rings or panels with one microcontroller? Almost any microcontroller, in fact!*

[MicroBlocks](https://microblocks.fun/) is a graphical-blocks language for programming microcontrollers. Its creators have played significant roles in developing the [Scratch](https://scratch.mit.edu/) and [Snap!](https://snap.berkeley.edu/) languages at MIT and at Berkeley, respectively.

Code written in MicroBlocks can be run on a very wide range of different hardware. A design goal for the language is that libraries developed for MicroBlocks would &ldquo;just work&rdquo; on any supported microcontroller.

What this means is that, when all goes well, and subject to available memory, the same code may run on an Arduino, an ESP32, a BBC micro:bit, and many others.
...
A typical way to begin a MicroBlocks program using this library could look like the following example.

![Example of a MicroBlocks code listing](block_images/when_started.png)
...
The List data type in MicroBlocks can contain an arbitrary (and changeable) number of data items including numbers, text, boolean (true-false) value and (this is the cool part) *other lists*.
...
In the above example code, a list named *DeviceChain* contains the data records for each physical device in the chain attached to the microcontroller. 

The order is important. Notice that the record for *Panel_8x8* comes first. 

The first device record in the list should represent the physical device actually connected to the microcontroller. For the arrangement shown in this example to work right, that physical panel should be attached to the controller.

Then the other data records follow in the same order as they follow outward from the controller in the physical chain.

All of the blocks for manipulating NeoPixels on a device take the device record as one of their inputs. For example, draw a solid, green square in the center of the 8x8 NeoPixel panel.

![image of a MicroBlocks code block for drawing a rectangle](block_images/example_rectangle.png)

You can draw many times into a device without affecting its visual presentation. These *NeoPixelDevices* blocks serve to prepare a device for viewing after you have completed a desired layout.

When the devices are ready for display, a single block updates all of them by working through the list of device data records. Staying with the above example:

![image of a MicroBlocks program block to display the chain of devices](block_images/example_display.png)

## Blocks Reference
The blocks are listed in the Palette area of the MicroBlocks editing window. Look for the library name in the lower-left area of the screen and click on it to reveal the list.
...
* [Set the Physical Connection](#the-physical-connection)
...
Only the specified device is affected by these blocks. The visual display of the device is not immediately updated. See the *display* block, described above.
...
The idea is to allow only one script to access a data object at any given time. 
